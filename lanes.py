...
def display_lines(image,lines):
    ...
    if lines is not None:
        for x1,y1,x2,y2  in lines:
            cv2.line(line_image,(x1,y1),(x2,y2),(255,0,0),10)#draws blue colored line
    return line_image

def region_of_interest(image):
    height = image.shape[0]
    polygons = np.array([
    [(-600,1000),(1250,800),(850,300)]
    ])
...
lines= cv2.HoughLinesP(cropped_image,2,np.pi/180, 250,np.array([]),minLineLength=60,maxLineGap=80 )#np.pi/180=1degree
